...
IP_REGEX = r'([1-9]|[1-9][0-9]|(1[0-9][0-9]|2[0-5][0-5]))' +\
        r'\.([1-9]|[1-9][0-9]|(1[0-9][0-9]|2[0-5][0-5]))' +\
        r'\.([1-9]|[1-9][0-9]|(1[0-9][0-9]|2[0-5][0-5]))' +\
        r'\.([1-9]|[1-9][0-9]|(1[0-9][0-9]|2[0-5][0-5]))'
DATE_REGEX = r'\[[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}' +\
        r'\.[0-9]{6}\]'
STATUS_REGEX = r'([2-5]0[01345])'
...
try:
    for log in sys.stdin:
        ...
        if match:
            ...
            # update logs
            log_tracker.update({
                'total_size': log_tracker.get('total_size', 0) + int(f_size),
    ...
    for status_code in status_codes:
        if log_tracker.get(status_code):
            # status code has been seen, and is being tracked
            print("{}: {}".format(status_code, log_tracker.get(status_code)))
    raise e
